...
5. pgo模块参与定位模式，重定位在pgo模块中，初始位姿提供两种方式：1. 通过rviz发布initial_pose话题，2. 读取一个给定的位姿文件。地图以及初始位姿文件在 launch 文件中指定
...
## 主要功能
（1）增加了pgo后端优化功能
（2）目前定位模式中，初始位姿还是需要通过rviz手动设置，或者发布initial_pose话题

## 建图执行：pgo中的 mapping.launch.py
```bash
    ros2 launch pgo mapping.launch.py
```
## 保存地图
